...
public class OrderBuyApiController {
    ...
    public String allBuyListData() throws Exception {
        ...
        if (userId > 0) {
            ...
            // 將使用者所有訂單資訊撈出來
            List<JSONObject> bookingOrders = orderBuyService.findOrdersByUserId(userId);
    ...
    public String insertOrder(@RequestBody String body) throws Exception {
        ...
        if (userId > 0 && body != null) {
            ...
            switch (orderStatus) {
                ...
                // 買方案下[確認入庫], 入庫和訂單同時完成
                case 6:
                    if (orderBuyService.productsInStockByOrderId(orderId) == true
                            && orderBuyService.productsOutStockByOrderId(orderId) == true) {
                ...
                // 買家將暫存訂單刪除
                case 8:
                    if (orderBuyService.deleteOrdersBean(ordersBean) == true) {
                        return "OK";
                    } else {
                        return "NG";
                    }
    ...
    // 給別人呼叫用的建構式
    public OrderBuyApiController(OrderBuyService orderBuyService, AccountsRepository accountsRepository,
